		...
		for(i=0;i<menunamecnt;i++)
		{
			...
			if ((i-topplc >= 0) && (i-topplc <= 36))
			{
				if (menuname[i][BUILD_MAXPATH] == 1)
				{
                    ...
                    #else
    					printext16(0L,((i-topplc)<<3)+16L,4,0,buffer,0);
                    #endif
				}
				else
				{
                    ...
                    #else
	    				printext16(0L,((i-topplc)<<3)+16L,7,0,buffer,0);
                    #endif
				}
			}
		}
...
void printcoords16(long posxe, long posye, short ange)
{
	...
	sprintf(snotbuf,"x=%ld y=%ld ang=%d",posxe,posye,ange);
	i = 0;
	...
	statusbar_printext16(8, 128, 11, 6, snotbuf,0);

	sprintf(snotbuf,"%d/%d sect. %d/%d walls %d/%d spri.",numsectors,MAXSECTORS,numwalls,MAXWALLS,numsprites,MAXSPRITES);
	i = 0;
	...
	statusbar_printext16(264, 128, 14, 6, snotbuf,0);
}
...
{
	long i, j, vidpos, vidpos2, wallnum, xdime, ydime, cnt, pinc;
		...
		if (wallnum < localartlookupnum)
		{
			...
			if ((tilesizx[wallnum] != 0) && (tilesizy[wallnum] != 0))
			{
				...
				if (localartlookupnum < MAXTILES)
				{
					dax = ((cnt%(xtiles<<gettilezoom))<<(6-gettilezoom));
					day = ((cnt/(xtiles<<gettilezoom))<<(6-gettilezoom));
					sprintf(snotbuf,"%d",localartfreq[cnt+pictopleft]);
...
int adjustmark(long *xplc, long *yplc, short danumwalls)
{
...
int checkautoinsert(long dax, long day, short danumwalls)
{
	long i, x1, y1, x2, y2;
...
short deletesector(short sucksect)
{
		...
		if (bad > 0)  /* Space */
		{
			if ((newnumwalls < numwalls) && (numwalls < MAXWALLS-1))
			{
				...
				split = 0;
          		...
          		clearbufbyte((void *) FP_OFF(&wall[newnumwalls]),sizeof(walltype),0L);
				...
				if ((split == 0) && (firstx == mousxplc) && (firsty == mousyplc) && (newnumwalls >= numwalls+3))
				{
					...
					if (suckwall == -1)  /* if no connections to other sectors */
					{
						...
						{              /* add island sector */
							...
							clearbufbyte((void *) FP_OFF(&sector[numsectors]),sizeof(sectortype),0L);
...
void editinput(void)
{
	...
	long i, j, k, templong=0, doubvel, changedir;
	...
	if (keystatus[0x57] > 0)  /* F11 - brightness */
	{
		...
		setbrightness((char) brightness, (unsigned char *) palette);
	}
	if (keystatus[88] > 0)   /* F12 */
	{
		screencapture("captxxxx.pcx",keystatus[0x2a]|keystatus[0x36]);
